...
lastChordRaw: "",
  ...
  chordTrackerMicStream: null,
  chordTrackerMicSource: null,
  chordTrackerAnalyser: null,
  chordTrackerRaf: null,
  chordTrackerLastDetectAt: 0,
  chordTrackerCandidate: "",
  chordTrackerCandidateCount: 0,
...
/***********************
Chord Tracker v1.2
- Restores a persistent Chord Tracker button next to Completed Projects
- Starts/stops AutoScroll with tracker mode
- Mic listens for a stable note/chord candidate and stamps into the active tick slot
***********************/
function ensureChordTrackerButton(){
  let btn = document.getElementById("chordTrackerBtn");
  if(!btn){
    const archiveBtn = document.getElementById("archiveBtn");
    if(archiveBtn && archiveBtn.parentElement){
      btn = document.createElement("button");
      btn.id = "chordTrackerBtn";
      btn.type = "button";
      btn.className = "chordTrackerBtn";
      btn.title = "Track chords/notes into the highlighted beat";
      btn.textContent = "Chord Tracker";
      archiveBtn.insertAdjacentElement("afterend", btn);
    }
  }
  if(btn){
    el.chordTrackerBtn = btn;
    if(btn.dataset.wired !== "1"){
      btn.dataset.wired = "1";
      btn.addEventListener("click", toggleChordTracker);
    }
  }
  return btn;
}
...
function updateChordTrackerUI(){
  const btn = ensureChordTrackerButton();
  if(!btn) return;
  const on = !!state.chordTrackerOn;
  btn.classList.toggle("active", on);
  btn.textContent = on ? "Tracking…" : "Chord Tracker";
  btn.title = on ? "Stop Chord Tracker" : "Track chords/notes into the highlighted beat";
...
function stopChordTrackerMic(){
  ...
  try{ state.chordTrackerMicSource && state.chordTrackerMicSource.disconnect(); }catch{}
  state.chordTrackerMicSource = null;
  state.chordTrackerAnalyser = null;
  try{
    if(state.chordTrackerMicStream){
      state.chordTrackerMicStream.getTracks().forEach(t => { try{ t.stop(); }catch{} });
    }
  }catch{}
  state.chordTrackerMicStream = null;
  state.chordTrackerCandidate = "";
  state.chordTrackerCandidateCount = 0;
}

function stopChordTracker(){
  state.chordTrackerOn = false;
  state.chordTrackerLastStamp = "";
  stopChordTrackerMic();
  if(state.autoScrollOn) setAutoScroll(false);
  updateChordTrackerUI();
}

async function startChordTracker(){
  state.chordTrackerOn = true;
  state.chordTrackerLastStamp = "";
  updateChordTrackerUI();
  if(!state.autoScrollOn) setAutoScroll(true);
  try{ await startChordTrackerMic(); }catch(err){
    console.warn("Chord Tracker mic failed", err);
  }
}

function toggleChordTracker(){
  if(state.chordTrackerOn) stopChordTracker();
  else startChordTracker();
}

function getChordTrackerSlot(){
  if(state.currentSection === "Full" && state.fullPerfMode){
    const beat4 = Number.isFinite(Number(state.fullPerfLastBeat4)) && Number(state.fullPerfLastBeat4) >= 0 ? Number(state.fullPerfLastBeat4) : 0;
    return clamp(Math.round(beat4) * 2, 0, 7);
  }
  const n = ((uiTick8() % 8) + 8) % 8;
  return clamp(n, 0, 7);
}

function getChordTrackerTarget(){
  if(!state.project || !state.project.sections) return null;
  const slot = getChordTrackerSlot();
  if(state.currentSection === "Full" && state.fullPerfMode){
    const items = Array.isArray(state.fullPerfItems) ? state.fullPerfItems : [];
    const idx = clamp(Number(state.fullPerfLineIndex)||0, 0, Math.max(0, items.length - 1));
    const item = items[idx];
    const sec = item && item.sectionKey ? item.sectionKey : "PAGE 1";
    const arr = ensureSectionArray(sec);
    const lineIdx = clamp(Number(item?.lineIndex)||0, 0, Math.max(0, arr.length - 1));
    const line = arr[lineIdx] || arr[0];
    return { section: sec, line, lineIndex: lineIdx, slot, card: null };
  }
  const sec = state.currentSection && state.currentSection !== "Full" ? state.currentSection : "PAGE 1";
  const arr = ensureSectionArray(sec);
  let card = state.autoScrollOn ? (getPlaybackCard() || getCardAtPlayLine() || getNearestVisibleCard()) : (getCardAtPlayLine() || getNearestVisibleCard());
  const cards = getCards();
  let lineIdx = card ? cards.indexOf(card) : 0;
  if(lineIdx < 0) lineIdx = 0;
  lineIdx = clamp(lineIdx, 0, Math.max(0, arr.length - 1));
  const line = arr[lineIdx] || arr[0];
  return { section: sec, line, lineIndex: lineIdx, slot, card };
}

function stampChordTrackerValue(raw){
  raw = String(raw || "").trim();
  if(!raw || !state.chordTrackerOn) return false;
  const target = getChordTrackerTarget();
  if(!target || !target.line) return false;
  const key = [target.section, target.line.id || target.lineIndex, target.slot, raw].join("|");
  if(state.chordTrackerLastStamp === key) return false;
  ...
  state.chordTrackerLastStamp = key;
  if(target.card){
    applyChordRowUiRefresh(target.card, target.line, Array.from(target.card.querySelectorAll(".noteCell")));
...
async function startChordTrackerMic(){
  if(state.chordTrackerMicStream || !navigator.mediaDevices?.getUserMedia) return;
  const ctx = ensureCtx();
  ...
  state.chordTrackerMicStream = stream;
  state.chordTrackerMicSource = src;
  state.chordTrackerAnalyser = analyser;
  state.chordTrackerRaf = requestAnimationFrame(chordTrackerDetectLoop);
}

function freqToNoteName(freq){
  const f = Number(freq);
  if(!Number.isFinite(f) || f < 45 || f > 1400) return "";
  const midi = Math.round(69 + 12 * Math.log2(f / 440));
  const names = ["C","C#","D","Eb","E","F","F#","G","Ab","A","Bb","B"];
  return names[((midi % 12) + 12) % 12];
}

function estimateFundamentalFromTimeDomain(buf, sampleRate){
  const n = buf.length;
  let rms = 0;
  ...
  if(bestOffset < 0 || bestCorr < 0.008) return 0;
  return sampleRate / bestOffset;
}

function chordTrackerDetectLoop(){
  if(!state.chordTrackerOn || !state.chordTrackerAnalyser || !state.ctx){
    state.chordTrackerRaf = null;
    return;
  }
  ...
  const freq = estimateFundamentalFromTimeDomain(buf, state.ctx.sampleRate || 44100);
  const note = freqToNoteName(freq);
  if(note){
    if(note === state.chordTrackerCandidate) state.chordTrackerCandidateCount = (state.chordTrackerCandidateCount || 0) + 1;
    else{ state.chordTrackerCandidate = note; state.chordTrackerCandidateCount = 1; }
    const t = Date.now();
    if(state.chordTrackerCandidateCount >= 4 && (t - (state.chordTrackerLastDetectAt || 0)) > 220){
      state.chordTrackerLastDetectAt = t;
      stampChordTrackerValue(note);
    }
  }else{
    state.chordTrackerCandidateCount = 0;
  }
...
try{ window.addEventListener("beforeunload", stopChordTrackerMic); }catch{}
...
function renderArchiveLoadedName(){
  // Loaded project name now lives in the main project picker for both active and archived songs.
  // Keep the old archive label hidden so the Chord Tracker button has stable space.
