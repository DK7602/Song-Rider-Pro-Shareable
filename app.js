...
function scrollCardIntoView(card){
  ...
  if(sb){
    ...
    // target = current scrollTop + delta - (sticky sheet header height + padding)
    const headerEl = sb.querySelector(".sheetHeader");
    const headerH = headerEl ? Math.ceil(headerEl.getBoundingClientRect().height || 0) : 0;
    const pad = headerH + 16; // ✅ enough room so card # + syllables never get clipped
