...
function chordTrackerLoop(){
  ...
  try{
    ...
    if(analyser){
      ...
      // Use the dedicated conservative piano chord engine when available.
      // This replaces the old in-app FFT guesser for live stamping, but leaves the
      // rest of the tracker UI/countdown/autoscroll/project logic untouched.
      let det = null;
      ...
      if(det){
        const cleanVal = chordTrackerCleanLabel(det.value || det.note || "");
        state.chordTrackerDetectedNote = det.note || (det.type === "note" ? cleanVal : "");
        state.chordTrackerDetectedChord = det.type === "chord" ? cleanVal : "";
        state.chordTrackerConfidence = det.confidence || 0;

        const minStamp = det.type === "chord" ? 0.64 : 0.72;
        if(cleanVal && det.confidence >= minStamp){
          stampChordTrackerChord(cleanVal);
        }
      }
      const t = performance.now();
      if(t - (Number(state.chordTrackerLastUiAt)||0) > CHORD_TRACKER_UI_MS){
...
async function startChordTrackerMic(){
  ...
  try{
    ...
    try{
      state.chordTrackerEngine = (window.SongRiderChordTracker && typeof window.SongRiderChordTracker.create === "function")
        ? window.SongRiderChordTracker.create()
        : null;
    }catch{ state.chordTrackerEngine = null; }
...
async function toggleChordTracker(){
  ...
  state.chordTrackerCandidateKey = "";
  state.chordTrackerCandidateCount = 0;
  try{ if(state.chordTrackerEngine && typeof state.chordTrackerEngine.reset === "function") state.chordTrackerEngine.reset(); }catch{}
  state.chordTrackerDetectedNote = "";
  state.chordTrackerDetectedChord = "";
  state.chordTrackerConfidence = 0;
