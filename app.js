...
/***********************
Chord Tracker v1
- Manual/app-played chord capture tied to the highlight tick
- Stores raw chords in the same line.notes[] model used by transpose, cards, and export
***********************/
function updateChordTrackerUI(){
  ...
  btn.title = on
    ? "Chord Tracker is armed — app-played chords stamp to the current highlight position"
    : "Start Chord Tracker";
}

function getChordTrackerSlotIndex(){
  const t = Number(uiTick8 ? uiTick8() : state.tick8) || 0;
  return clamp(((Math.round(t) % 8) + 8) % 8, 0, 7);
}
...
function getChordTrackerTarget(){
  ...
  if(state.currentSection === "Full"){
    ...
    let idx = Number(state.fullPerfLineIndex);
    if(!Number.isFinite(idx) || idx < 0 || idx >= items.length){
      idx = items.length ? clamp(findFirstVisibleFullPerfLineIndex(), 0, items.length - 1) : 0;
    }
    const item = items[idx] || null;
    if(item && item.sectionKey){
      const arr = ensureSectionArray(item.sectionKey);
      while(arr.length <= item.lineIndex) arr.push(newLine());
      return { section:item.sectionKey, arr, line:arr[item.lineIndex], lineIndex:item.lineIndex, slot, source:"full" };
    }
  }

  const section = (state.currentSection && state.currentSection !== "Full")
    ? state.currentSection
    : (state.chordTrackerSection || "PAGE 1");
  const arr = ensureSectionArray(section);
  ...
  while(arr.length <= lineIndex) arr.push(newLine());
  return { section, arr, line:arr[lineIndex], lineIndex, slot, source:"card", card };
}

function stampChordTrackerChord(rawChord){
  if(!state.chordTrackerOn || !state.project) return false;
  const raw = String(rawChord || "").trim();
  if(!raw || isTieToken(raw) || !parseChordToken(raw)) return false;

  const target = getChordTrackerTarget();
  if(!target || !target.line) return false;

  const stampKey = [state.project.id, target.section, target.lineIndex, target.slot, raw, Math.floor((Date.now() || 0) / 120)].join("|");
  ...
  try{
    if(state.currentSection === "Full"){
      const perf = Array.isArray(state.fullPerfItems) ? state.fullPerfItems[state.fullPerfLineIndex] : null;
      if(perf && Array.isArray(perf.playSlots)) perf.playSlots[target.slot] = raw;
    }else if(target.card){
      const inputs = Array.from(target.card.querySelectorAll(".noteCell"));
      applyChordRowUiRefresh(target.card, target.line, inputs);
    }
    refreshDisplayedNoteCells();
    updateKeyFromAllNotes();
    updateFullIfVisible();
...
function toggleChordTracker(){
  state.chordTrackerOn = !state.chordTrackerOn;
  state.chordTrackerSection = (state.currentSection && state.currentSection !== "Full") ? state.currentSection : "";
  state.chordTrackerLastStamp = "";

  // Chord Tracker is tied to the visible highlight tic. Start AutoScroll if needed,
  // but do not change drums, recording, or instrument choices.
  if(state.chordTrackerOn && !state.autoScrollOn){
    try{ setAutoScroll(true); }catch{}
  }
...
function renderProjectsDropdown(){
  ...
  if(el.projectPickerLabel){
    ...
    el.projectPickerLabel.title = state.project && state.project.archived ? (label + " (Completed Projects)") : label;
  }
