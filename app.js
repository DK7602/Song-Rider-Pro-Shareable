...
/***********************
Chord Tracker v3 — clean labels + sharper minor chord detection
- Uses music-mode microphone input, not phone-call processing
- Detects the strongest single note with autocorrelation
- Builds a rolling pitch-class profile for chord inference
- Tracker owns the AutoScroll session it starts; turning tracker off stops it too
***********************/
const CHORD_TRACKER_MIN_FREQ = 73;   // low D2-ish; avoids rumble
const CHORD_TRACKER_MAX_FREQ = 1320; // E6-ish; guitar + vocal range safety
const CHORD_TRACKER_FRAME_KEEP_MS = 950;
const CHORD_TRACKER_STAMP_COOLDOWN_MS = 520;
const CHORD_TRACKER_UI_MS = 140;
...
function updateChordTrackerUI(){
  ...
  if(on){
    const chord = String(state.chordTrackerDetectedChord || "").trim();
    const note = String(state.chordTrackerDetectedNote || "").trim();
...
function chordTrackerCleanLabel(value){
  ...
  // Clean note+octave labels (E4, G3) and power-chord labels (E5) out of the live tracker.
  const octave = out.match(/^([A-Ga-g](?:#|b)?)-?\d$/);
  if(octave) return octave[1][0].toUpperCase() + octave[1].slice(1);

  const power = out.match(/^([A-Ga-g](?:#|b)?)5$/);
  if(power) return power[1][0].toUpperCase() + power[1].slice(1);

  const m = out.match(/^([A-Ga-g])([#b]?)(.*)$/);
  if(!m) return out;
  const root = m[1].toUpperCase() + (m[2] || "");
  let rest = m[3] || "";
  rest = rest.replace(/^min$/i,"m").replace(/^minor$/i,"m");
  return root + rest;
}
...
function chordTrackerDetectPitch(buf, sampleRate){
  ...
  // Trim very quiet edges for cleaner correlation.
  let start = 0, end = n - 1;
  const th = Math.max(0.012, rms * 0.6);
  for(let i=0;i<n/2;i++){ if(Math.abs(buf[i]) > th){ start = i; break; } }
  for(let i=1;i<n/2;i++){ if(Math.abs(buf[n-i]) > th){ end = n - i; break; } }
  const size = end - start;
  if(size < 256) return null;

  const minLag = Math.max(2, Math.floor(sampleRate / CHORD_TRACKER_MAX_FREQ));
  const maxLag = Math.min(size - 2, Math.ceil(sampleRate / CHORD_TRACKER_MIN_FREQ));
  ...
  for(let lag=minLag; lag<=maxLag; lag++){
    ...
    // Prefer the first strong peak after correlation starts rising again.
    if(corr > 0.62 && corr > prevCorr){
      foundGood = true;
      if(corr > bestCorr){ bestCorr = corr; bestLag = lag; }
    }else if(foundGood && corr < bestCorr * 0.82){
      break;
    }
    if(corr > bestCorr){ bestCorr = corr; bestLag = lag; }
    prevCorr = corr;
  ...
  try{
    ...
    if(Math.abs(denom) > 1e-6) lag = bestLag + ((c2 - c0) / denom);
  }catch{}
  ...
  const confidence = clamp(bestCorr * (1 - Math.min(40, cents) / 120), 0, 1);
  return { freq, midi, pc: ((Math.round(midi)%12)+12)%12, note: chordTrackerNoteNameFromMidi(midi), confidence, rms };
}
...
function chordTrackerBuildChromaFromAnalyser(analyser){
  ...
  for(let i=1;i<data.length;i++){
    ...
    // Harmonic fold-down: energy at overtones helps the likely fundamental pitch class.
    for(let h=1; h<=4; h++){
      const f0 = freq / h;
...
function chordTrackerAggregate(){
  ...
  for(const fr of frames){
    ...
    if(Number.isFinite(fr.pc)) noteVotes[fr.pc] += Math.max(0.05, Number(fr.pitchConfidence)||0.05);
  }
  if(chromaFrames){
    for(let i=0;i<12;i++) chroma[i] /= chromaFrames;
  }
  // Blend stable single-note detection into chroma. This helps guitar chords whose root is quiet.
  const maxVote = Math.max(...noteVotes);
  if(maxVote > 0){
    for(let i=0;i<12;i++) chroma[i] += 0.38 * (noteVotes[i] / maxVote);
  }
  const max = Math.max(...chroma);
  if(max <= 0) return null;
  ...
  return { chroma, note, noteConfidence: clamp(maxVote / Math.max(0.001, frames.length), 0, 1), frameCount: frames.length };
}

function chordTrackerChordTemplates(){
  return [
    // No “5” power-chord output in the live tracker; it looked like random numbers after notes.
    ...
    { suffix:"maj7", pcs:[0,4,7,11], need:[0,4,11], label:"maj7" },
    { suffix:"m7", pcs:[0,3,7,10], need:[0,3,10], label:"minor7" },
    { suffix:"add9", pcs:[0,4,7,14], need:[0,4,7], label:"add9" },
    { suffix:"madd9", pcs:[0,3,7,14], need:[0,3,7], label:"madd9" },
    { suffix:"dim", pcs:[0,3,6], need:[0,3,6], label:"dim" },
    { suffix:"aug", pcs:[0,4,8], need:[0,4,8], label:"aug" }
  ];
}

function chordTrackerInferChord(chroma){
  ...
  for(let root=0; root<12; root++){
    ...
    const minorThird = val(root + 3);
    const majorThird = val(root + 4);
    ...
    for(const tpl of templates){
      ...
      const outside = chroma.reduce((sum, v, pc) => sum + (pcsAbs.includes(pc) ? 0 : Math.max(0, Number(v)||0)), 0) / 12;

      let score = (hit * 0.55) + (needed * 0.42) + (rootEnergy * 0.14) + (fifth * 0.10) - (outside * 0.42);

      const minorFamily = tpl.suffix === "m" || tpl.suffix === "m7" || tpl.suffix === "madd9" || tpl.suffix === "dim";
      const majorFamily = tpl.suffix === "" || tpl.suffix === "7" || tpl.suffix === "maj7" || tpl.suffix === "add9";

      if(minorFamily){
        score += minorThird * 0.30;
        score -= majorThird * 0.34;
        score += 0.055;
      }else if(majorFamily){
        score += majorThird * 0.26;
        score -= minorThird * 0.31;
      }

      if(tpl.suffix === "sus2") score += val(root+2)*0.22 - Math.max(minorThird, majorThird)*0.20;
      if(tpl.suffix === "sus4") score += val(root+5)*0.22 - Math.max(minorThird, majorThird)*0.20;
      if(tpl.pcs.length > 3) score -= 0.035;

      if(!best || score > best.score){
        best = { root, suffix:tpl.suffix, score, hit, needed, rootEnergy, fifthEnergy:fifth, minorThird, majorThird, pcs:pcsAbs };
      }
    }
  }

  if(!best) return null;
  const sorted = chroma.map((v,pc)=>({v:Number(v)||0,pc})).sort((a,b)=>b.v-a.v);
  const strongCount = sorted.filter(x => x.v >= 0.38).length;

  const minorFamily = best.suffix === "m" || best.suffix === "m7" || best.suffix === "madd9" || best.suffix === "dim";
  const majorFamily = best.suffix === "" || best.suffix === "7" || best.suffix === "maj7" || best.suffix === "add9";
  const thirdEnergy = minorFamily ? best.minorThird : majorFamily ? best.majorThird : Math.max(best.minorThird, best.majorThird);
  const oppositeThird = minorFamily ? best.majorThird : majorFamily ? best.minorThird : 0;

  const identityOk = (best.rootEnergy >= 0.24 || best.fifthEnergy >= 0.34 || best.needed >= 0.46);
  const thirdOk = (!minorFamily && !majorFamily) ? true : (thirdEnergy >= 0.24 && thirdEnergy >= oppositeThird + 0.045);
  const minScore = strongCount <= 1 ? 0.68 : 0.42;

  if(!identityOk || !thirdOk || best.needed < 0.38 || best.score < minScore) return null;

  let suffix = best.suffix;
  if(["7","maj7","m7","add9","madd9"].includes(suffix)){
    const extPc = suffix.includes("maj7") ? (best.root+11)%12 : suffix.includes("7") ? (best.root+10)%12 : (best.root+2)%12;
    if(val(extPc) < 0.42) suffix = suffix.startsWith("m") ? "m" : "";
  }

  const chord = chordTrackerCleanLabel(`${CHORD_TRACKER_ROOT_NAMES[best.root]}${suffix}`);
  const confidence = clamp((best.score + best.needed + thirdEnergy + best.fifthEnergy) / 2.65, 0, 1);
  return { chord, confidence, root:best.root, suffix };
}

function chordTrackerCurrentDetection(){
  const agg = chordTrackerAggregate();
  if(!agg || agg.frameCount < 4) return null;
  const chord = chordTrackerInferChord(agg.chroma);
  if(chord && chord.confidence >= 0.45){
    return { type:"chord", value: chordTrackerCleanLabel(chord.chord), confidence: chord.confidence, note: chordTrackerCleanLabel(agg.note) };
  }
  if(agg.note && agg.noteConfidence >= 0.34){
    return { type:"note", value: chordTrackerCleanLabel(agg.note), confidence: agg.noteConfidence, note: chordTrackerCleanLabel(agg.note) };
  }
  return null;
}
...
function chordTrackerLoop(){
  ...
  try{
    ...
    if(analyser){
      ...
      const det = chordTrackerCurrentDetection();
      if(det){
        state.chordTrackerDetectedNote = det.note || (det.type === "note" ? det.value : "");
        state.chordTrackerDetectedChord = det.type === "chord" ? det.value : "";
        state.chordTrackerConfidence = det.confidence || 0;
        if(det.value && det.confidence >= (det.type === "chord" ? 0.48 : 0.42)){
          stampChordTrackerChord(det.value);
        }
      }
      const t = performance.now();
...
async function startChordTrackerMic(){
  ...
  // If recording is active, do not steal/stop its mic stream. Use a separate stream only when needed.
  let stream = null;
  try{
    stream = await navigator.mediaDevices.getUserMedia(MUSIC_MODE_CONSTRAINTS);
  }catch(err){
    console.error("Chord tracker mic failed", err);
...
function renderArchiveLoadedName(){
  // Completed/archived project names should live in the Projects picker,
  // not beside the Chord Tracker button. Keep the legacy span hidden
  // so older markup cannot leak the title into the tracker row.
...
function renderProjectsDropdown(){
  ...
  if(el.projectPickerLabel){
    // Show the loaded project name in the Projects window/button for both
    // active projects and completed/archived projects.
