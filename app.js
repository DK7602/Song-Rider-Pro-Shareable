...
const CHORD_TRACKER_FRAME_KEEP_MS = 1800;
const CHORD_TRACKER_STAMP_COOLDOWN_MS = 640;
...
function chordTrackerBuildChromaFromAnalyser(analyser){
  ...
  for(let i=1;i<data.length;i++){
    const db = data[i];
    if(!Number.isFinite(db) || db < -94) continue;
    const freq = i * sr / fft;
    if(freq < 70 || freq > 1900) continue;
    const amp = Math.pow(10, db / 20);
    if(amp <= 0) continue;

    // Fold harmonics back to pitch classes. This makes strummed guitar chords usable.
    for(let h=1; h<=5; h++){
      const f0 = freq / h;
      if(f0 < CHORD_TRACKER_MIN_FREQ || f0 > CHORD_TRACKER_MAX_FREQ) continue;
      const midi = chordTrackerFreqToMidi(f0);
      if(midi === null) continue;
      const near = Math.round(midi);
      const cents = Math.abs((midi - near) * 100);
      if(cents > 46) continue;
      const pc = ((near % 12) + 12) % 12;
      const w = amp * (1 / Math.sqrt(h)) * (1 - cents / 70);
      chroma[pc] += w;
      total += w;
    }
  }
...
function chordTrackerAggregate(){
  ...
  if(maxVote > 0){
    for(let i=0;i<12;i++) chroma[i] += 0.55 * (noteVotes[i] / maxVote);
  }
  const max = Math.max(...chroma);
  if(max <= 0) return null;
...
function chordTrackerChordTemplates(){
  return [
    { suffix:"", pcs:[0,4,7], need:[0,4,7], label:"major" },
    { suffix:"m", pcs:[0,3,7], need:[0,3,7], label:"minor" },
    { suffix:"sus2", pcs:[0,2,7], need:[0,2,7], label:"sus2" },
    { suffix:"sus4", pcs:[0,5,7], need:[0,5,7], label:"sus4" },
    { suffix:"7", pcs:[0,4,7,10], need:[0,4,10], label:"dominant7" },
    { suffix:"m7", pcs:[0,3,7,10], need:[0,3,10], label:"minor7" },
    { suffix:"add9", pcs:[0,2,4,7], need:[0,4,7], label:"add9" },
    { suffix:"madd9", pcs:[0,2,3,7], need:[0,3,7], label:"madd9" }
  ];
}
...
function chordTrackerInferChord(input){
  ...
  for(let root=0; root<12; root++){
    for(const tpl of templates){
      ...
      let score = (needed * 0.55) + (hit * 0.28) + (rootEnergy * 0.22) + (fifth * 0.08) - (outside * 0.38);

      // Important: sharpen major vs minor without blocking detection.
      if(tpl.suffix === "m" || tpl.suffix === "m7"){
        score += (thirdMinor - thirdMajor) * 0.30;
      }else if(tpl.suffix === ""){
        score += (thirdMajor - thirdMinor) * 0.26;
      }

      // Prefer the current strongest single-note pitch as the root when close.
      if(input && input.note){
        const noteRoot = CHORD_TRACKER_ROOT_NAMES.indexOf(input.note);
        if(noteRoot === root) score += 0.07;
      }

      if(!best || score > best.score){
        best = { root, suffix:tpl.suffix, score, hit, needed, rootEnergy, thirdMajor, thirdMinor, fifth, pcs:pcsAbs };
      }
    }
  }

  if(!best) return null;
  const strongCount = chroma.filter(v => v >= 0.34).length;
  ...
  const thirdOk = isMinorish
    ? (best.thirdMinor >= 0.20 && best.thirdMinor >= best.thirdMajor * 0.72)
    : (isMajorish ? (best.thirdMajor >= 0.20 && best.thirdMajor >= best.thirdMinor * 0.70) : true);
  const chordRich = strongCount >= 2 || pitchSpread >= 2;
  const minNeeded = chordRich ? 0.25 : 0.62;
  const minScore = chordRich ? 0.24 : 0.64;
  if(best.needed < minNeeded || best.score < minScore || !thirdOk) return null;

  let suffix = best.suffix;
  if(suffix === "7" && val(best.root + 10) < 0.42) suffix = "";
  if(suffix === "m7" && val(best.root + 10) < 0.42) suffix = "m";
  if(suffix === "add9" && val(best.root + 2) < 0.48) suffix = "";
  if(suffix === "madd9" && val(best.root + 2) < 0.48) suffix = "m";

  const chord = chordTrackerCleanLabel(CHORD_TRACKER_ROOT_NAMES[best.root] + suffix);
  const confidence = clamp((best.score + best.needed + best.rootEnergy + (chordRich ? 0.16 : 0)) / 2.04, 0, 1);
...
function getChordTrackerTarget(){
  ...
  idx = clamp(idx, 0, Math.max(0, arr.length - 1));
  if(!arr[idx]) arr[idx] = newLine();
  return { section, arr, idx, line: arr[idx], slot };
}
...
function stampChordTrackerChord(rawChord){
  ...
  if(state.currentSection !== "Full"){
    try{
      const card = getCards()[target.idx];
...
function chordTrackerLoop(){
  ...
  try{
    ...
    if(analyser){
      ...
      let det = chordTrackerCurrentDetection();
      det = chordTrackerStabilizeDetection(det);
      if(det){
        state.chordTrackerDetectedNote = det.note || (det.type === "note" ? det.value : "");
        state.chordTrackerDetectedChord = det.type === "chord" ? chordTrackerCleanLabel(det.value) : "";
        ...
        const minStamp = det.type === "chord" ? 0.24 : (looksLikeChordWindow ? 0.42 : 0.20);
