...
function chordTrackerLoop(){
  ...
  try{
    ...
    if(analyser){
      ...
      // Use only the dedicated chord-tracker.js engine for live detection.
      // The older in-app fallback is intentionally disabled because it is the piece
      // that was guessing relative/wrong chords like G -> Em, C -> Am, and D -> F.
      let det = null;
      ...
      if(det){
        ...
        const minStamp = 0.56;
        if(det.type === "chord" && det.stable === true && cleanVal && det.confidence >= minStamp){
