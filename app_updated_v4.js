...
***********************/
const SECTIONS = ["Full","INTRO","VERSE 1","CHORUS 1","VERSE 2","CHORUS 2","VERSE 3","BRIDGE","CHORUS 3","OUTRO"];

// ✅ Optional extra pages (user-created after OUTRO)
...
function defaultProject(name="New Song"){
  ...
  return {
    ...
    extraSections: [],   // ✅ pages after OUTRO
...
function updateKeyFromAllNotes(){
  // Key detection: chord-first, with a heuristic to prefer the most "tonic-like" chord.
  // Fixes cases like: Em – Am – Bm7 – Em (should be E min, not A min)
  ...
  const extractChordTokens = (s) => {
    const t = String(s||"")
      .replace(/[\[\]\(\)\{\}]/g," ")
      .replace(/\u00A0/g," ")
      .trim();
    if(!t) return [];
    // Split on whitespace and common separators, keep # b + and digits
    return t.split(/[^A-Za-z0-9#b+\/]+/).filter(Boolean);
  };

  getFullEditSections().forEach(sec => {
    const arr = (state.project && state.project.sections && state.project.sections[sec]) ? state.project.sections[sec] : [];
    arr.forEach(line => {
      // 1) Chords (beats boxes) — strong signal
      const beats = Array.isArray(line?.beats) ? line.beats : [];
      beats.forEach(b => {
        const toks = extractChordTokens(b);
        toks.forEach(tok => {
          const ch = parseChordToken(tok);
          if(!ch) return;

          // capture first/last chord encountered
          if(!firstChord) firstChord = { rootPC: ch.rootPC, triad: ch.triad };
          lastChord = { rootPC: ch.rootPC, triad: ch.triad };

          // root statistics (for tonic heuristic)
          const rp = ((ch.rootPC % 12) + 12) % 12;
          rootCount[rp] += 1;
          if(ch.triad === "min" || ch.triad === "dim") minorRootCount[rp] += 1;
          else if(ch.triad === "maj") majorRootCount[rp] += 1;

          // Weight chord tones: root strongest, then other tones
          addPC(ch.rootPC, 3.0);
          (Array.isArray(ch.intervals) ? ch.intervals : []).forEach(iv => {
            if(iv === 0) return;
            addPC(ch.rootPC + iv, 1.4);
          });
          if(ch.bassPC !== null) addPC(ch.bassPC, 1.0);
        });
      });

      // 2) Melody notes (note boxes) — light signal (helps when no chords)
      const notes = Array.isArray(line?.notes) ? line.notes : [];
      notes.forEach(tok => {
  ...
  const isStrongBoundary = (pc) => {
    ...
    if(firstChord && firstChord.rootPC === pc) return true;
    if(lastChord && lastChord.rootPC === pc) return true;
    return false;
  };

  // Prefer: last chord's minor root when it also dominates (very common songwriting resolution)
  ...
  // If Krumhansl says relative key (e.g., A min) but the most frequent/boundary chord is a different minor root (e.g., E min), pick that.
  if(k.mode === "min" && topMinorV >= 2){
    // A "tonic-like" candidate: most frequent minor-root chord
    const cand = topMinorRoot;
    const candV = minorRootCount[cand] || 0;

    // override if candidate is clearly stronger and appears on boundary (start/end)
    if(cand !== k.pc && candV >= (minorRootCount[k.pc] || 0) + 1 && isStrongBoundary(cand)){
      k = { pc: cand, mode: "min", score: k.score };
    }
  }

  // If still mismatched and another root strongly dominates AND is on boundary, use it
  ...
  // Apply transpose display (capo/steps) to the KEY label
...
function renderSheetActions(){
  ...
  addBtn.addEventListener("click", (e) => {
    ...
    const order = getFullEditSections(); // INTRO..OUTRO + extras (if any)
    ...
    if(i >= 0 && i < order.length - 1){
      ...
      // After OUTRO (or unknown) -> create a new extra page
