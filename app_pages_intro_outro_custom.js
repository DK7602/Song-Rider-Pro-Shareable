...
function renderSheetActions(){
  // Page header controls (+ next page, × delete blank page)
  ...
  if(isExtra && el.sheetTitle){
    ...
    const commitRename = () => {
      ...
      const all = getAllSections().map(s=>s.toUpperCase());
      if(oldName.toUpperCase() !== newName.toUpperCase() && all.includes(newName.toUpperCase())){
      ...
      editProject("renamePage", () => {
        ...
        // rename in extraSections
        state.project.extraSections = (state.project.extraSections||[]).map(s => (String(s).toUpperCase() === oldName.toUpperCase()) ? newName : s);

        // rename in enabledSections
        state.project.enabledSections = (state.project.enabledSections||[]).map(s => (String(s).toUpperCase() === oldName.toUpperCase()) ? newName : s);
  ...
  addBtn.addEventListener("click", (e) => {
    ...
    const order = getFullEditSections();
    const cur = state.currentSection;
    const i = order.indexOf(cur);

    // if current not found (shouldn't), start after INTRO
    let nextName = null;
    if(i >= 0 && i < order.length - 1){
      ...
      editProject("addExtraPage", () => {
        ...
        state.project.extraSections.push(nextName);
        state.project.sections[nextName] = [newLine()];
        state.project.enabledSections = (state.project.enabledSections||[]);
        state.project.enabledSections.push(nextName);
        // keep Full in sync (adds heading)
        syncFullTextFromSections();
      });
    }
